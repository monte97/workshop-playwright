...
# 5. Global Setup: Parti da uno Stato Noto
...
### Il Problema
```
Test 1 (PASS) → Crea dati
Test 2 (FAIL) → Dati non puliti!
Test 3 → Database "sporco"
```

**Soluzione:** Reset completo prima di tutto
...
### Vantaggi
- Stato pulito garantito
- Test paralleli sicuri
...
# Riepilogo: Le 5 Best Practice

<div class="text-base">

| Best Practice | Problema Risolto | Strumento Playwright |
|:---|:---|:---|
| **API per Setup/Teardown** | Test lenti e fragili | `request` / `APIRequestContext` |
| **Dati Unici** | Test falliscono in parallelo | `faker-js` o `Date.now()` |
| **Fixtures** | Logica setup/teardown sparsa | `test.extend` |
| **StorageState** | Login lento in ogni test | `storageState` + Dipendenze |
| **Global Setup** | Database "sporco" da test falliti | `globalSetup` |

</div>

---

# Esempio Completo: E-Commerce Test
...
export const test = base.extend<{
  ...
  utenteLoggato: async ({ request }, use) => {
    const email = faker.internet.email();
...
```js
// carrello.spec.ts
import { test } from './fixtures/utente';

test('aggiunge prodotto al carrello',
  async ({ page, utenteLoggato }) => {

  // Già loggato! Setup fatto via API!
  ...
  await expect(
    page.getByText('Aggiunto al carrello')
  ).toBeVisible();
...
```js
test('crea post', ...);
test('modifica post', ...); // Dipende dal primo!
```
...
### ✅ Test Isolati
```js
test('crea e modifica post', ...);
// Tutto in un test, isolato
```
